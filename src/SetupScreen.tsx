...
export default function SetupScreen({ onStartMatch, onBack }: { onStartMatch: (settings: AppSettings) => void; onBack?: () => void }) {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
            ...
            {/* Header */}
            <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 14, borderBottomWidth: 1, borderBottomColor: colors.divider }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <View>
                        <Text style={{ color: colors.textPrimary, fontSize: 22, fontWeight: '700' }}>Match Setup</Text>
                        <Text style={{ color: colors.textMuted, fontSize: 12, marginTop: 2 }}>Configure your match settings</Text>
                    </View>
                </View>
            </View>

            <ScrollView style={{ padding: 20 }} showsVerticalScrollIndicator={false}>
                <Text style={{ color: colors.textMuted, fontWeight: '700', fontSize: 11, letterSpacing: 1.2, marginBottom: 10, marginTop: 4 }}>TEAMS</Text>
                <View style={{ backgroundColor: colors.card, borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: colors.cardBorder }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 4 }}>
                        <Ionicons name="flag-outline" size={18} color={colors.accent} style={{ marginRight: 10 }} />
                        <TextInput style={{ flex: 1, fontSize: 15, color: colors.textPrimary, paddingVertical: 8 }} placeholder="Host Team" value={hostTeam} onChangeText={setHostTeam} placeholderTextColor={colors.inputPlaceholder} />
                    </View>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 4 }}>
                        ...
                        <TextInput style={{ flex: 1, fontSize: 15, color: colors.textPrimary, paddingVertical: 8 }} placeholder="Visitor Team" value={visitorTeam} onChangeText={setVisitorTeam} placeholderTextColor={colors.inputPlaceholder} />
                    </View>
                </View>

                {/* Toss Won By Section */}
                <Text style={{ color: colors.textMuted, fontWeight: '700', fontSize: 11, letterSpacing: 1.2, marginBottom: 10, marginTop: 4 }}>TOSS WON BY</Text>
                <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
                    <TouchableOpacity
                        style={[{ flex: 1, flexDirection: 'row', paddingVertical: 13, paddingHorizontal: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, toss === 'host' ? { backgroundColor: colors.accent, borderWidth: 1, borderColor: colors.accent } : { backgroundColor: colors.chipUnselectedBg, borderWidth: 1, borderColor: colors.chipUnselectedBorder }]}
                        onPress={() => setToss('host')}
                    >
                        <Text
                            style={[{ fontSize: 14 }, toss === 'host' ? { color: colors.chipSelectedTextOnAccent, fontWeight: '700' } : { color: colors.chipUnselectedText, fontWeight: '500' }]}
                            numberOfLines={1}
                        >
                    ...
                    <TouchableOpacity
                        style={[{ flex: 1, flexDirection: 'row', paddingVertical: 13, paddingHorizontal: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, toss === 'visitor' ? { backgroundColor: colors.accent, borderWidth: 1, borderColor: colors.accent } : { backgroundColor: colors.chipUnselectedBg, borderWidth: 1, borderColor: colors.chipUnselectedBorder }]}
                        onPress={() => setToss('visitor')}
                    >
                        <Text
                            style={[{ fontSize: 14 }, toss === 'visitor' ? { color: colors.chipSelectedTextOnAccent, fontWeight: '700' } : { color: colors.chipUnselectedText, fontWeight: '500' }]}
                            numberOfLines={1}
                        >
                ...
                {/* Opted To */}
                <Text style={{ color: colors.textMuted, fontWeight: '700', fontSize: 11, letterSpacing: 1.2, marginBottom: 10, marginTop: 4 }}>OPTED TO</Text>
                <View style={{ flexDirection: 'row', gap: 10, marginBottom: 20 }}>
                    <TouchableOpacity
                        style={[{ flex: 1, flexDirection: 'row', paddingVertical: 13, paddingHorizontal: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, opted === 'bat' ? { backgroundColor: colors.accent, borderWidth: 1, borderColor: colors.accent } : { backgroundColor: colors.chipUnselectedBg, borderWidth: 1, borderColor: colors.chipUnselectedBorder }]}
                        onPress={() => setOpted('bat')}
                    >
                        <Ionicons name="baseball-outline" size={16} color={opted === 'bat' ? colors.chipSelectedTextOnAccent : colors.chipUnselectedText} style={{ marginRight: 6 }} />
                        <Text style={[{ fontSize: 14 }, opted === 'bat' ? { color: colors.chipSelectedTextOnAccent, fontWeight: '700' } : { color: colors.chipUnselectedText, fontWeight: '500' }]}>
                            Bat
                        </Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                        style={[{ flex: 1, flexDirection: 'row', paddingVertical: 13, paddingHorizontal: 14, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, opted === 'bowl' ? { backgroundColor: colors.accent, borderWidth: 1, borderColor: colors.accent } : { backgroundColor: colors.chipUnselectedBg, borderWidth: 1, borderColor: colors.chipUnselectedBorder }]}
                        onPress={() => setOpted('bowl')}
                    >
                        <Ionicons name="ellipse-outline" size={16} color={opted === 'bowl' ? colors.chipSelectedTextOnAccent : colors.chipUnselectedText} style={{ marginRight: 6 }} />
                        <Text style={[{ fontSize: 14 }, opted === 'bowl' ? { color: colors.chipSelectedTextOnAccent, fontWeight: '700' } : { color: colors.chipUnselectedText, fontWeight: '500' }]}>
                            Bowl
                        </Text>
                    </TouchableOpacity>
                </View>

                <Text style={{ color: colors.textMuted, fontWeight: '700', fontSize: 11, letterSpacing: 1.2, marginBottom: 10, marginTop: 4 }}>TOTAL OVERS</Text>
                <View style={{ backgroundColor: colors.card, borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: colors.cardBorder }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 20, paddingVertical: 4 }}>
                        <TouchableOpacity style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: colors.buttonBg, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.cardBorder }} onPress={handleDecrementOvers}>
                            <Ionicons name="remove" size={18} color={colors.textPrimary} />
                        </TouchableOpacity>

                        <TextInput
                            style={{ fontSize: 28, fontWeight: '800', color: colors.textPrimary, minWidth: 60, textAlign: 'center' }}
                            value={overs}
                        ...
                        <TouchableOpacity style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: colors.buttonBg, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.cardBorder }} onPress={handleIncrementOvers}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'center', gap: 10, marginTop: 14 }}>
                        {[5, 10, 20, 50].map((num) => {
                            ...
                            return (
                                <TouchableOpacity
                                    key={num}
                                    style={[{ paddingVertical: 8, paddingHorizontal: 18, borderRadius: 20, borderWidth: 1 }, isSelected ? { backgroundColor: colors.accent, borderColor: colors.accent } : { backgroundColor: colors.chipUnselectedBg, borderColor: colors.chipUnselectedBorder }]}
                ...
                {/* Advanced Settings */}
                <TouchableOpacity style={{ backgroundColor: colors.card, borderWidth: 1, borderColor: colors.cardBorder, padding: 16, borderRadius: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }} onPress={() => setIsAdvancedExpanded(true)}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                        <Ionicons name="settings-outline" size={18} color={colors.textSecondary} />
                        <Text style={{ color: colors.textSecondary, fontWeight: '600', fontSize: 14 }}>Advanced Settings</Text>
                    </View>
                    <Ionicons name="chevron-forward" size={18} color={colors.textMuted} />
                </TouchableOpacity>

                {/* Load Test Scenario */}
                <TouchableOpacity style={{ backgroundColor: colors.accentPurpleBg, borderWidth: 1, borderColor: colors.accentPurpleBorder, padding: 15, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }} onPress={handleLoadTest}>
                    <Ionicons name="flask-outline" size={18} color={colors.accentPurple} style={{ marginRight: 8 }} />
                    <Text style={{ color: '#A78BFA', fontWeight: '600', fontSize: 14 }}>Load Test Scenario (Last Over)</Text>
                </TouchableOpacity>

                {/* Start Match */}
                <TouchableOpacity style={{ backgroundColor: colors.accent, padding: 16, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 16, elevation: 4, shadowColor: colors.accent, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 }} onPress={handleStart}>
                    <Ionicons name="play" size={18} color={colors.chipSelectedTextOnAccent} style={{ marginRight: 8 }} />
                    <Text style={{ color: colors.chipSelectedTextOnAccent, fontWeight: '800', fontSize: 16 }}>Start Match</Text>
                </TouchableOpacity>
