...
interface TeamPlayerProfileScreenProps {
    route?: any;
    navigation?: any;
    player?: any;
    onBack?: () => void;
...
export default function TeamPlayerProfileScreen({
    route,
    navigation,
    player: propPlayer,
    onBack,
}: TeamPlayerProfileScreenProps) {
  ...
  const StatCard = ({ label, value }: { label: string; value: string | number }) => (
    ...
    const playerData = route?.params?.player || propPlayer;
    ...
    const getRoleSubtitle = () => {
        const role = playerData?.playingRole || 'Batter';
        
        let batAbbr = 'RHB';
        if (playerData?.battingHand === 'Left Handed' || playerData?.battingStyle === 'Left Handed') {
            batAbbr = 'LHB';
        }

        let bowlAbbr = 'None';
        const category = playerData?.bowlingCategory || playerData?.bowlingStyle;
        if (category === 'Fast' || category === 'Medium Fast') {
            const arm = playerData?.bowlingArm === 'Left Arm' ? 'LAF' : 'RAF';
            bowlAbbr = category === 'Fast' ? arm : `${arm}M`;
        } else if (category === 'Spin' || category === 'Spinner') {
            if (playerData?.spinType?.includes('Leg')) bowlAbbr = 'RALB';
            else if (playerData?.spinType?.includes('Chinaman')) bowlAbbr = 'LACB';
            else if (playerData?.spinType?.includes('Orthodox')) bowlAbbr = 'SLA';
            else bowlAbbr = 'RAOB';
        }
    ...
    const displayName = playerData?.displayName || playerData?.fullName || playerData?.name || 'Player Profile';
                ...
                {activeTab === 'Overview' && (
                    <View style={styles.overviewCard}>
                        ...
                        <View style={styles.gridRowFull}>
                            ...
                            <Text style={styles.valueText}>{playerData?.fullName || playerData?.name || 'Not specified'}</Text>
                        </View>
                        ...
                        <View style={styles.gridRowTwoCol}>
                            <View style={styles.gridCol}>
                                ...
                                <Text style={styles.valueText}>
                                    {playerData?.battingHand || playerData?.battingStyle || 'Right Handed'}
                                </Text>
                            </View>
                            ...
                            <View style={styles.gridCol}>
                                ...
                                <Text style={styles.valueText}>
                                    {playerData?.bowlingCategory || playerData?.bowlingStyle || 'None'}
                                    {playerData?.bowlingArm ? ` (${playerData.bowlingArm})` : ''}
                                    {playerData?.spinType ? ` (${playerData.spinType})` : ''}
                                </Text>
                            </View>
                        </View>
                        ...
                        <View style={styles.gridRowTwoCol}>
                            <View style={styles.gridCol}>
                                ...
                                <Text style={styles.valueText}>{playerData?.playingRole || 'Batter'}</Text>
                            </View>
                            ...
                            <View style={styles.gridCol}>
                                ...
                                <Text style={styles.valueText}>{playerData?.currentTeam || 'None'}</Text>
                            </View>
                        </View>
                        ...
                        <View style={styles.gridRowFull}>
                            ...
                            <Text style={styles.valueText}>{playerData?.previousTeams || 'None'}</Text>
                        </View>
                    </View>
                )}
                ...
                {activeTab === 'Matches' && (
                    <View style={styles.emptyStateCard}>
                        ...
                        <Text style={styles.emptySubtitle}>
                            Match logs, scorecard summaries, and past game performances for this player will be listed here.
                        </Text>
                    </View>
                )}
            </ScrollView>
